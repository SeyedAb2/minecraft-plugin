...
public class CommandLoad implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sender instanceof Player) {
            ...
            if (StaticGame.game != null && (StaticGame.game.isAccepted() || StaticGame.game.isLoaded()) && !isSure) {
                try {
            ...
            StaticGame.game = new Game(player, null);
            if (isSure && player.equals(loader)) {
                try {
                    StaticGame.game.load(id, player);
                    loader = null;
                    isSure = false;
                    id = 0;
                    return true;
                ...
                try {
                    StaticGame.game.load(Integer.parseInt(args[0]), player);
                } catch (NumberFormatException e) {
                    System.out.println("Invalid input. Only numbers will be accepted.");
                    return false;
                }
